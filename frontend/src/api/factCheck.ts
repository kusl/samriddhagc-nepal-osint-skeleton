...
export async function submitStatementFactCheck(statement: string): Promise<StatementFactCheck> {
  const response = await apiClient.post('/fact-check/statement', { statement });
  return response.data;
}

/** Get all custom statement fact-checks for the current user. */
export async function getUserStatements(limit = 20): Promise<StatementFactCheck[]> {
  const response = await apiClient.get('/fact-check/statements', { params: { limit } });
  return response.data;
}

// ── Pending Story Requests ──

export interface PendingStoryRequest {
  story_id: string;
  story_title: string | null;
  story_source: string | null;
  story_url: string | null;
...
/** Get user's pending story-based fact-check requests (not yet completed). */
export async function getMyStoryRequests(): Promise<PendingStoryRequest[]> {
  const response = await apiClient.get('/fact-check/my-requests');
  return response.data;
}
