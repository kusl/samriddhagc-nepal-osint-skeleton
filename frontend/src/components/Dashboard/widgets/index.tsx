...
export function SocialWidget() {
  ...
  // When Reddit is selected, force political category
  const effectiveCategory = activeSource === 'reddit' && activeCategory === 'all' ? 'political' : activeCategory;
      ...
      <div style={{ display: 'flex', gap: 3, padding: '4px 8px', borderBottom: '1px solid var(--border-subtle)', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {CATEGORY_OPTIONS.map(({ key, label }) => (
          <button
            key={key}
            type="button"
            className={`widget-action ${activeCategory === key ? 'active' : ''}`}
            onClick={() => setActiveCategory(key)}
            style={{ fontSize: 8, whiteSpace: 'nowrap', padding: '1px 5px' }}
          >
            {label}
          </button>
        ))}
        ...
        <select
          value={activeSource}
          onChange={(event) => setActiveSource(event.target.value as SocialSource)}
          style={{
            background: 'var(--bg-surface)',
            border: '1px solid var(--border-subtle)',
            color: 'var(--text-primary)',
        ...
        {TIME_OPTIONS.map(({ key, label }) => (
          <button
            key={key}
            type="button"
            className={`widget-action ${activeHours === key ? 'active' : ''}`}
            onClick={() => setActiveHours(key)}
        ...
        <button
          type="button"
          className={`widget-action ${nepalOnly ? 'active' : ''}`}
          onClick={() => setNepalOnly((prev) => !prev)}
          style={{ fontSize: 8, padding: '1px 4px' }}
        >
          Nepal
        </button>
        <button
          type="button"
          className={`widget-action ${groundReports ? 'active' : ''}`}
          onClick={() => setGroundReports((prev) => !prev)}
          style={{ fontSize: 8, padding: '1px 4px' }}
        >
          Ground
        </button>
