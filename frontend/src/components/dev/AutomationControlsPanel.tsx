...
import { Activity, Bot, PauseCircle, PlayCircle, RotateCcw } from 'lucide-react'
import {
              ...
              <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-xs text-white/45">
                    ...
                    <div className="text-right text-white/80">{data.telemetry.embeddings.total.toLocaleString()}</div>
                    <div>Created in 24h</div>
                    <div className="text-right text-white/80">{data.telemetry.embeddings.created_24h.toLocaleString()}</div>
                    <div>Recent stories embedded</div>
                    <div className="text-right text-white/80">{data.telemetry.embeddings.recent_stories_embedded_24h.toLocaleString()}</div>
                    <div>Estimated OpenAI calls / 24h</div>
                    <div className="text-right text-white/80">{data.telemetry.embeddings.estimated_openai_embedding_calls_24h.toLocaleString()}</div>
                    <div>Batch size</div>
                    <div className="text-right text-white/80">{data.telemetry.embeddings.batch_size.toLocaleString()}</div>
                    <div>Last embedding activity</div>
                    <div className="text-right text-white/80">{formatTimestamp(data.telemetry.embeddings.last_embedding_at)}</div>
                  </div>
                  <div className="mt-3">
                    <div className="text-[11px] uppercase tracking-[0.18em] text-white/35">Model mix</div>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {Object.entries(data.telemetry.embeddings.model_counts).map(([model, count]) => (
                        <span
                          key={model}
                ...
                <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-xs text-white/45">
                    ...
                    <div className="text-right text-white/80">{data.telemetry.clustering.total_clusters.toLocaleString()}</div>
                    <div>Updated in 24h</div>
                    <div className="text-right text-white/80">{data.telemetry.clustering.updated_clusters_24h.toLocaleString()}</div>
                    <div>Recent stories clustered</div>
                    <div className="text-right text-white/80">{data.telemetry.clustering.recent_stories_clustered_24h.toLocaleString()}</div>
                    <div>Last cluster activity</div>
                    <div className="text-right text-white/80">{formatTimestamp(data.telemetry.clustering.last_cluster_activity_at)}</div>
                  </div>
                </div>
              </div>
            )}
