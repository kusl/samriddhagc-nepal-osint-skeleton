 ...
 * Driven by the Province Anomaly Agent only.
 */
...
import { useProvinceAnomalies } from '../../../api/hooks';
...
function ProvinceRow({ prov, anomaly, isExpanded, onToggle }: {
  prov: { id: number; name: string; short: string };
  anomaly: ProvinceAnomalyData | null;
  isExpanded: boolean;
  onToggle: () => void;
}) {
  const level = anomaly?.threat_level || 'LOW';
  const threat = THREAT_CONFIG[level] || THREAT_CONFIG.LOW;
  const trajectory = anomaly?.threat_trajectory || 'STABLE';
  ...
  const summary = anomaly?.summary || null;
  const storyCount = anomaly?.story_count ?? 0;
...
export const ProvinceMonitorWidget = memo(function ProvinceMonitorWidget() {
  const { data: anomalyData, isLoading, error, refetch } = useProvinceAnomalies();
