...
class TwitterService:
            ...
            for tweet in result.tweets:
                try:
                    ...
                    if created:
                        ...
                        await self.enrich_and_link_tweet(db_tweet.id)
                    else:
                        stats["updated"] += 1
    ...
    async def _classify_tweet(self, tweet: Tweet) -> None:
        """Classify tweet for Nepal relevance."""
        try:
            relevance_service = RelevanceService()

            # Check Nepal relevance
            result = relevance_service.check_nepal_relevance(tweet.text)

            # Update tweet
            await self.repo.mark_processed(
                tweet_id=tweet.id,
                nepal_relevance=result.relevance.value if result.relevance else None,
                is_relevant=result.relevance is not None and result.relevance.value != "NOT_RELEVANT",
                relevance_score=result.confidence,
            )

        except Exception as e:
            logger.error(f"Error classifying tweet {tweet.id}: {e}")
            # Mark as processed even on error to avoid reprocessing
    ...
    async def enrich_and_link_tweet(self, tweet_id) -> None:
        """
        Enrich a tweet with geography and link it to the strongest nearby story event.

        This keeps social posts as a secondary evidence stream attached to events,
        rather than allowing them to define events on their own.
        """
        tweet = await self.repo.get_tweet_by_id(tweet_id)
        if not tweet:
            return

        tweet_features = self.feature_extractor.extract(
            title=tweet.text[:240],
            summary=tweet.text,
            content=tweet.text,
            story_id=str(tweet.id),
            published_at=tweet.tweeted_at or tweet.fetched_at,
        )
        ...
        best_geo = 0.0
        best_entity = 0.0
        event_time = tweet.tweeted_at or tweet.fetched_at or datetime.now(timezone.utc)

        stmt = (
            select(Story)
            .options(selectinload(Story.features), selectinload(Story.cluster))
            .where(Story.nepal_relevance.in_(["NEPAL_DOMESTIC", "NEPAL_NEIGHBOR"]))
            .order_by(Story.published_at.desc().nullslast())
            .limit(120)
        )
        if tweet.category:
            stmt = stmt.where(Story.category == tweet.category)
        result = await self.db.execute(stmt)
        candidate_stories = list(result.scalars().all())

        for story in candidate_stories:
            story_time = story.published_at or story.created_at
            if not story_time:
                continue
            time_diff = abs((event_time - story_time).total_seconds()) / 3600.0
            if time_diff > 72:
                continue

            story_features = story.features or self.feature_extractor.extract(
                title=story.title,
            ...
            score = self.similarity_engine.compute_hybrid_similarity_with_blocking(
                tweet_features,
                story_features,
                None,
                None,
            ...
            if score.overall > best_score:
                ...
                best_geo = score.geo_similarity
                best_entity = score.entity_overlap

        linked_story_id = None
        if best_story and best_score >= 0.58 and (best_geo >= 0.34 or best_entity >= 0.18):
            linked_story_id = best_story.id

        districts = tweet_features.districts or (best_story.districts if best_story else []) or []
        provinces = tweet_features.provinces or (best_story.provinces if best_story else []) or []
        await self.repo.update_intelligence_fields(
            tweet.id,
            districts=districts,
            provinces=provinces,
            story_id=linked_story_id,
        )

    async def backfill_recent_tweet_links(self, hours: int = 168, limit: int = 500) -> Dict[str, int]:
        """Re-enrich and relink recent relevant tweets to story events."""
        cutoff = datetime.now(timezone.utc) - timedelta(hours=hours)
        result = await self.db.execute(
            select(Tweet)
            .where(
                Tweet.fetched_at >= cutoff,
                Tweet.nepal_relevance.in_(["NEPAL_DOMESTIC", "NEPAL_NEIGHBOR"]),
            )
        ...
        processed = 0
        for tweet in tweets:
            before_story_id = tweet.story_id
            await self.enrich_and_link_tweet(tweet.id)
            refreshed = await self.repo.get_tweet_by_id(tweet.id)
            processed += 1
            if refreshed and refreshed.story_id and refreshed.story_id != before_story_id:
                linked += 1
        return {"tweets_scanned": len(tweets), "tweets_processed": processed, "tweets_linked": linked}
