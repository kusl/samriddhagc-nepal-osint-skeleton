...
class HybridSimilarityScore:
    ...
    Components:
    - semantic: embedding cosine similarity (45% weight)
    - lexical: MinHash Jaccard similarity (20% weight)
    - structural: geography + entity + temporal + category/event compatibility (35% weight)
    """
...
class SimilarityEngine:
    ...
    # Lexical catches exact overlap but should not dominate event merges.
    HYBRID_LEXICAL_WEIGHT = 0.20
    # Structural is heavier because event identity depends on geo/entities/time.
    HYBRID_STRUCTURAL_WEIGHT = 0.35

    # Structural sub-component weights
    STRUCTURAL_ENTITY_WEIGHT = 0.45
    STRUCTURAL_GEO_WEIGHT = 0.30
    STRUCTURAL_TEMPORAL_WEIGHT = 0.15
    STRUCTURAL_CATEGORY_WEIGHT = 0.10
    ...
    def _compute_geo_similarity(
        ...
        """
        # Combine normalized geography signals.
        geo1 = (
            set(features1.districts)
            | set(features1.provinces)
            | set(features1.municipalities)
            | set(features1.constituencies)
        )
        geo2 = (
            set(features2.districts)
            | set(features2.provinces)
            | set(features2.municipalities)
            | set(features2.constituencies)
        )
    ...
    def _compute_entity_similarity(
        ...
        # Normalized entities and key terms
        terms1 = (
            set(features1.key_terms)
            | set(features1.named_people)
            | set(features1.named_orgs)
            | set(features1.named_parties)
            | set(features1.named_infrastructure)
        )
        terms2 = (
            set(features2.key_terms)
            | set(features2.named_people)
            | set(features2.named_orgs)
            | set(features2.named_parties)
            | set(features2.named_infrastructure)
        )
    ...
    def compute_hybrid_similarity(
        ...
        # 3d. Category/event compatibility
        category_compatibility = self._compute_category_event_compatibility(
            category1, category2, features1.event_type, features2.event_type
        )
        ...
        structural_sim = (
            ...
            self.STRUCTURAL_CATEGORY_WEIGHT * category_compatibility
        )
    ...
    def _compute_category_event_compatibility(
        self,
        category1: Optional[str],
        category2: Optional[str],
        event_type1: Optional[str],
        event_type2: Optional[str],
    ) -> float:
        score = 0.0
        if category1 and category2 and category1 == category2:
            score += 0.6
        if event_type1 and event_type2 and event_type1 == event_type2:
            score += 0.4
