...
class AnnouncementService:
    ...
    async def _emit_security_graph_evidence(
        ...
        *,
        ...
        source_object = await self.db.scalar(
            select(KBObject).where(KBObject.canonical_key == f"security_source:{source_id}")
        )
        if not source_object:
            source_object = KBObject(
                object_type="security_source",
                canonical_key=f"security_source:{source_id}",
                title=source_name,
                description=f"Official security service source {source_domain}",
                attributes={"domain": source_domain, "source_id": source_id},
                source_count=1,
                confidence=0.9,
            ...
            source_object.source_count += 1
            source_object.confidence = min(1.0, max(source_object.confidence, 0.9))

        event_key = f"security_event:{announcement_id}"
        event_object = await self.db.scalar(select(KBObject).where(KBObject.canonical_key == event_key))
        if not event_object:
            event_object = KBObject(
            ...
            self.db.add(event_object)
            await self.db.flush()

        link = await self.db.scalar(
            select(KBLink)
            .where(KBLink.source_object_id == source_object.id)
            .where(KBLink.target_object_id == event_object.id)
            .where(KBLink.predicate == "PUBLISHED_SECURITY_NOTICE")
        )
        if not link:
            link = KBLink(
                source_object_id=source_object.id,
                target_object_id=event_object.id,
                predicate="PUBLISHED_SECURITY_NOTICE",
                confidence=0.85,
                source_count=1,
                verification_status=AnalystVerificationStatus.CANDIDATE,
            )
            ...
            link.source_count += 1
            link.confidence = min(1.0, max(link.confidence, 0.85))

        self.db.add(
            KBEvidenceRef(
                owner_type=ProvenanceOwnerType.OBJECT,
                owner_id=str(event_object.id),
                evidence_type="govt_announcement",
                evidence_id=announcement_id,
                source_url=url,
                source_key=f"announcement:{announcement_id}",
                source_name=source_name,
                source_classification=SourceClassification.OFFICIAL,
                confidence=0.9,
                excerpt=title,
                evidence_metadata={"category": category, "source_domain": source_domain},
            )
        )
        self.db.add(
            KBEvidenceRef(
                ...
                evidence_id=announcement_id,
                source_url=url,
                source_key=f"announcement_link:{announcement_id}",
                source_name=source_name,
                source_classification=SourceClassification.OFFICIAL,
                confidence=0.88,
                excerpt=title,
                evidence_metadata={"predicate": "PUBLISHED_SECURITY_NOTICE", "location_hint": location_hint},
            )
        )
