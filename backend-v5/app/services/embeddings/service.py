...
class EmbeddingService:
    ...
    def _uses_openai_embeddings(self) -> bool:
        return (
            self.settings.openai_embedding_enabled
            and self.settings.embedding_model_key in {"openai-3-small", "openai-3-large"}
        )
    ...
    async def _ensure_embedder_matches_schema(self) -> None:
        ...
        if self._uses_openai_embeddings() and dim == self.settings.openai_embedding_dimensions:
            return
    ...
    async def batch_generate_embeddings(
        ...
        if self._uses_openai_embeddings():
            all_embeddings = await self.openai_runtime.embed_texts(
                all_texts,
                model=self.settings.openai_embedding_model,
                dimensions=self.settings.openai_embedding_dimensions,
                user_scope="story-embeddings",
