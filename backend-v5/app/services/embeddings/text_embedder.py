...
class OpenAITextEmbedder(MultilingualTextEmbedder):
    ...
    def __init__(self, model_key: str = "openai-3-small"):
        if model_key not in {"openai-3-small", "openai-3-large"}:
            model_key = "openai-3-small"
...
def get_embedder(model_key: str = "minilm") -> MultilingualTextEmbedder:
    ...
    with _embedder_lock:
        if model_key not in _embedder_instances:
            if model_key in {"openai-3-small", "openai-3-large"}:
                _embedder_instances[model_key] = OpenAITextEmbedder(model_key=model_key)
            else:
