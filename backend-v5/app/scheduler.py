...
from app.services.loss_sync_service import LossSyncService
...
LOSS_SYNC_INTERVAL = 900        # 15 minutes (BIPAD casualty/damage assessments)
...
GOVT_DECISION_INTERVAL = 21600  # 6 hours (OpenAI government decision extraction)
CABINET_ACTION_INTERVAL = 21600  # 6 hours (OpenAI cabinet action evidence tracking)
...
RIVER_CLEANUP_INTERVAL = settings.river_cleanup_interval_seconds
PROCUREMENT_POLL_INTERVAL = 43200  # 12 hours (Bolpatra contracts change slowly)
...
async def run_clustering():
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            stats = await service.cluster_stories(hours=24, min_cluster_size=2)
            logger.info(
...
async def sync_flood_loss():
    """Reconcile BIPAD casualty and damage assessments (every 15 min).

    BIPAD publishes an incident immediately with an empty loss record and fills
    the figures in over the following hours, so ingest alone would leave every
    death toll and damage estimate at zero. This job pulls the real numbers and
    keeps re-checking recent incidents while their assessments are still moving.
    """
    logger.info("Syncing BIPAD loss assessments...")
    try:
        async with AsyncSessionLocal() as db:
            result = await LossSyncService(db).sync(limit=400)
            logger.info(
                "Loss sync complete: %d updated of %d considered (+%d deaths)",
                result.updated, result.considered, result.deaths_added,
            )
            if result.updated:
                await invalidate_kpi_cache()
    except Exception as e:
        logger.exception(f"Error syncing BIPAD loss data: {e}")
...
async def sync_flood_live():
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            logger.info(
                "Flood live sync complete: toll=%s sitreps=%s media=%s",
                (result.get("toll") or {}).get("deaths"),
                (result.get("sitreps") or {}).get("created"),
...
async def cleanup_river_monitoring():
    """Prune old river readings on a low-frequency cadence."""
    logger.info("Cleaning up old river readings...")
    try:
        async with AsyncSessionLocal() as db:
            service = RiverMonitoringService(db)
            deleted = await service.cleanup_old_readings(days=settings.river_retention_days)
            logger.info("River cleanup complete: %s readings deleted", deleted)
    except Exception as e:
        logger.exception(f"Error cleaning up river data: {e}")
...
async def poll_procurement_contracts():
    """Poll Bolpatra e-GP for awarded contract updates on a low-cost cadence."""
    logger.info("Polling Bolpatra for procurement contracts...")
    try:
        async with AsyncSessionLocal() as db:
            service = ProcurementService(db)
            stats = await service.ingest_contracts_with_options(
                page_size=5000,
                scraper_delay=0.8,
                refresh_ocr_linkage=False,
            )
            logger.info(
                "Procurement poll complete: %s fetched, %s new, %s updated, %s errors",
                stats.get("fetched", 0),
                stats.get("new", 0),
                stats.get("updated", 0),
        ...
        logger.exception(f"Error polling procurement contracts: {e}")
...
async def poll_reddit():
    """Scrape Reddit public JSON API for Nepal-related subreddits (every 2 hours)."""
    try:
        async with AsyncSessionLocal() as db:
            service = RedditService(db)
            logger.info("Scraping Reddit (r/Nepal, r/NepalStock, etc.)...")
            result = await service.scrape_all()

            subs = result.get("subreddits_scraped", 0)
            searches = result.get("searches_scraped", 0)
            fetched = result.get("posts_fetched", 0)
            new = result.get("new_posts", 0)
            errors = result.get("errors", [])

            logger.info(
                f"Reddit: {subs} subs + {searches} searches scraped, "
                f"{fetched} posts fetched, {new} new"
            )
            if errors:
                logger.warning(f"Reddit errors: {errors}")

    except Exception as e:
        logger.exception(f"Error polling Reddit: {e}")
...
async def run_analyst_agent():
    """Run the Narada Analyst Agent to produce a situation brief (every 8 hours)."""
...
async def run_province_anomaly_agent():
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            control_service = EditorialControlService(db)
            if not await control_service.is_enabled("province_anomaly_agent"):
                logger.info("Province anomaly agent paused, skipping scheduled run")
                return
            await control_service.mark_run_started("province_anomaly_agent")
            agent = ProvinceAnomalyAgent(db=db, hours=12)
            run = await agent.run()
            logger.info(
            ...
            await control_service.mark_run_finished(
                "province_anomaly_agent",
                success=(run.status == "completed"),
                error=None if run.status == "completed" else f"Province anomaly run returned status={run.status}",
            )
    except Exception as e:
        try:
            async with AsyncSessionLocal() as db:
                control_service = EditorialControlService(db)
                await control_service.mark_run_finished("province_anomaly_agent", success=False, error=str(e))
        except Exception:
            logger.exception("Failed to mark province anomaly agent error")
        logger.exception(f"Error in province anomaly agent: {e}")


async def run_openai_vps_briefing():
    """Run the VPS OpenAI briefing cycle at fixed Nepal-time intervals."""
    logger.info("Running VPS OpenAI briefing cycle...")
    try:
        from app.services.editorial_control_service import EditorialControlService
        from app.services.openai_briefing_service import OpenAIBriefingService

        async with AsyncSessionLocal() as db:
            control_service = EditorialControlService(db)
            if not await control_service.is_enabled("analyst_brief_generation"):
                logger.info("Analyst brief generation paused, skipping VPS OpenAI briefing run")
                return
            await control_service.mark_run_started("analyst_brief_generation")
            service = OpenAIBriefingService(db=db)
            result = await service.run()
            logger.info(
                "VPS OpenAI briefing complete: run #%s, stories=%s, political_tweets=%s, period=%s..%s",
                result["brief"].run_number,
                result["stories_analyzed"],
        ...
        try:
            ...
            logger.exception("Failed to mark VPS OpenAI briefing error")
        logger.exception("Error in VPS OpenAI briefing cycle: %s", e)


async def run_govt_decision_generation():
    """Run the government decision extraction pipeline every 6 hours."""
    logger.info("Running government decision extraction...")
    try:
        from app.services.editorial_control_service import EditorialControlService
        ...
        async with AsyncSessionLocal() as db:
            control_service = EditorialControlService(db)
            if not await control_service.is_enabled("govt_decision_generation"):
                logger.info("Government decision generation paused, skipping scheduled run")
                return
            await control_service.mark_run_started("govt_decision_generation")
            service = GovtDecisionService(db)
            stats = await service.run_generation(force=True)
            logger.info(
                "Government decision extraction complete: processed=%s created=%s updated=%s skipped=%s",
                stats.get("processed", 0),
                stats.get("created", 0),
                stats.get("updated", 0),
                stats.get("skipped", 0),
        ...
        try:
            async with AsyncSessionLocal() as db:
                control_service = EditorialControlService(db)
                await control_service.mark_run_finished("govt_decision_generation", success=False, error=str(e))
        except Exception:
            logger.exception("Failed to mark govt decision generation error")
        logger.exception("Error in government decision extraction: %s", e)


async def run_cabinet_action_tracking():
    """Run evidence tracking for the cabinet 100-day action programme."""
    logger.info("Running cabinet action evidence tracking...")
    try:
        from app.services.editorial_control_service import EditorialControlService
        ...
        async with AsyncSessionLocal() as db:
            control_service = EditorialControlService(db)
            if not await control_service.is_enabled("cabinet_action_tracking"):
                logger.info("Cabinet action tracking paused, skipping scheduled run")
                return
            await control_service.mark_run_started("cabinet_action_tracking")
            service = CabinetActionService(db)
            stats = await service.run_tracking(force=True)
            logger.info(
                "Cabinet action tracking complete: processed=%s matched=%s applied=%s queued=%s",
                stats.get("processed", 0),
                stats.get("matched", 0),
                stats.get("applied", 0),
                stats.get("queued", 0),
        ...
        try:
            async with AsyncSessionLocal() as db:
                control_service = EditorialControlService(db)
                await control_service.mark_run_finished("cabinet_action_tracking", success=False, error=str(e))
        except Exception:
            logger.exception("Failed to mark cabinet action tracking error")
        logger.exception("Error in cabinet action evidence tracking: %s", e)
...
def start_scheduler():
    ...
    # Clustering every 2 hours over the recent 24h window.
    scheduler.add_job(
        run_clustering,
    ...
    # Casualty/damage assessments lag the incident report, so this runs on its
    # own slower cadence rather than inside the 5-minute incident poll.
    scheduler.add_job(
        sync_flood_loss,
        trigger=IntervalTrigger(seconds=LOSS_SYNC_INTERVAL),
        id="sync_flood_loss",
        name="Sync BIPAD Loss Assessments",
        replace_existing=True,
        next_run_time=now,
    )

    # The official flood feeds move on their own cadence — NDRRMA revises the
    # situation board a few times a day — so this polls faster than the loss
    # sync but writes only when a revision passes the service's guards.
    scheduler.add_job(
        sync_flood_live,
    ...
    if settings.river_monitoring_enabled:
        ...
        scheduler.add_job(
            cleanup_river_monitoring,
            trigger=IntervalTrigger(seconds=RIVER_CLEANUP_INTERVAL),
            id="cleanup_river",
            name="Cleanup River Readings",
            replace_existing=True,
    ...
    scheduler.add_job(
        poll_procurement_contracts,
        trigger=IntervalTrigger(seconds=PROCUREMENT_POLL_INTERVAL),
        id="poll_procurement_contracts",
        name="Poll Bolpatra Procurement Contracts",
        replace_existing=True,
    ...
    scheduler.add_job(
        ...
        trigger=IntervalTrigger(seconds=GOVT_DECISION_INTERVAL),
        id="run_govt_decision_generation",
        name="Run Government Decision Extraction",
        replace_existing=True,
        next_run_time=now + timedelta(minutes=10),
    )

    scheduler.add_job(
        run_cabinet_action_tracking,
        trigger=IntervalTrigger(seconds=CABINET_ACTION_INTERVAL),
        id="run_cabinet_action_tracking",
        name="Run Cabinet Action Tracking",
        replace_existing=True,
    ...
    # Reddit scraper every 2 hours (public JSON API, no auth needed)
    scheduler.add_job(
        poll_reddit,
        trigger=IntervalTrigger(seconds=REDDIT_POLL_INTERVAL),
        id="poll_reddit",
        name="Scrape Reddit",
        replace_existing=True,
    )
