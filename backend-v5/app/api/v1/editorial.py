...
async def get_automation_controls(
    ...
    total_embeddings = (
        await db.execute(select(func.count()).select_from(StoryEmbedding))
    ).scalar_one()
    embeddings_created_24h = (
        await db.execute(
            select(func.count())
            .select_from(StoryEmbedding)
            .where(StoryEmbedding.created_at >= datetime.now(timezone.utc) - timedelta(hours=24))
        )
    ).scalar_one()
    last_embedding_at = (
        await db.execute(select(func.max(StoryEmbedding.created_at)))
    ).scalar_one_or_none()
    embedding_model_counts_rows = (
        await db.execute(
            select(StoryEmbedding.model_name, func.count())
            .group_by(StoryEmbedding.model_name)
            .order_by(func.count().desc())
        )
    ).all()
    embedded_stories_24h = (
        await db.execute(
            select(func.count(func.distinct(StoryEmbedding.story_id)))
            .join(Story, Story.id == StoryEmbedding.story_id)
            .where(Story.created_at >= datetime.now(timezone.utc) - timedelta(hours=24))
        )
    ).scalar_one()
    total_clusters = (
        await db.execute(select(func.count()).select_from(StoryCluster))
    ).scalar_one()
    clusters_updated_24h = (
        await db.execute(
            select(func.count())
            .select_from(StoryCluster)
            .where(StoryCluster.last_updated >= datetime.now(timezone.utc) - timedelta(hours=24))
        )
    ).scalar_one()
    clustered_recent_stories = (
        await db.execute(
            select(func.count())
            .select_from(Story)
            .where(
                Story.created_at >= datetime.now(timezone.utc) - timedelta(hours=24),
                Story.cluster_id.is_not(None),
            )
        )
    ).scalar_one()
    embedding_limit = 400
    estimated_embedding_calls_24h = (
        (embeddings_created_24h + embedding_limit - 1) // embedding_limit if embeddings_created_24h else 0
    )
    return {
        "items": [serialize_control(control) for control in controls],
        "openai": {
            "status": (
                "healthy"
        ...
        "telemetry": {
            "embeddings": {
                ...
                "created_24h": embeddings_created_24h,
                "recent_stories_embedded_24h": embedded_stories_24h,
                "last_embedding_at": last_embedding_at,
                "model_counts": {
                    model_name: count for model_name, count in embedding_model_counts_rows
                },
                "estimated_openai_embedding_calls_24h": estimated_embedding_calls_24h,
                "batch_size": embedding_limit,
            },
            "clustering": {
                "total_clusters": total_clusters,
                "updated_clusters_24h": clusters_updated_24h,
                "recent_stories_clustered_24h": clustered_recent_stories,
                "last_cluster_activity_at": (
                    await db.execute(select(func.max(StoryCluster.last_updated)))
                ).scalar_one_or_none(),
            },
        },
    }
