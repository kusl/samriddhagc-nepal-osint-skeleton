...
async def get_related_stories(
    ...
    Uses existing semantic embeddings via pgvector nearest-neighbor search.
    Does not generate embeddings on demand for request-time traffic.
    """
    repo = StoryRepository(db)
    ...
    if not source_story.embedding or (
        source_story.embedding.embedding_vector is None and
        source_story.embedding.embedding is None
    ):
        return {
            "source_story_id": str(story_id),
            "similar_stories": [],
            "total_found": 0,
            "model": "semantic-cache-miss",
        }
    ...
    related_rows = await db.execute(
        select(Story)
        .options(selectinload(Story.cluster))
        .where(Story.id.in_(related_ids))
    )
    related_map = {story.id: story for story in related_rows.scalars().all()}

    similar_items = []
    active_model = source_story.embedding.model_name if source_story.embedding else "semantic"
        ...
        similar_items.append(
            {
                ...
                "event_cluster_id": str(cluster.id) if cluster else None,
                "event_type": cluster.event_type if cluster else None,
                "primary_province": cluster.primary_province if cluster else None,
                "primary_district": cluster.primary_district if cluster else None,
                "main_entities": cluster.main_entities if cluster and cluster.main_entities else [],
