...
async def get_map_events(
    ...
    if include_news:
        ...
        # Get recent stories using only schema-stable fields. The production DB
        # can lag newer StoryFeature columns, so the live map path must not
        # depend on eager-loading story.features.
        ...
        # Use raw story-row frequency as the corroboration/source fallback so the
        # map does not depend on the story_clusters table being schema-current.
        ...
        for story in stories:
            ...
            features.append(MapFeature(
                properties=MapFeatureProperties(
                    ...
                    confidence=0.75 if tactical and tactical.municipality else 0.6,
