...
async def ingest_tweets(
    ...
    for t in payload.tweets:
        try:
            ...
            if classify:
                try:
                    result = relevance_service.classify(
                        title=t.text[:200], content=t.text,
                        source_id=f"ingest_{t.source_query or 'unknown'}",
                    )
                    nepal_relevance = result.level.value
                    is_relevant = result.score >= 0.3
                    cat = result.category.value if result.category else None
            ...
            try:
                await twitter_service.enrich_and_link_tweet(db_tweet.id)
            except Exception as e:
                logger.warning(f"Tweet intelligence enrichment failed for {t.tweet_id}: {e}")
