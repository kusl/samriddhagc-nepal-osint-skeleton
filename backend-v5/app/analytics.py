...
_feed_feature_extractor = get_feature_extractor()
_FEED_TITLE_IGNORE = {
    "सरकार", "मन्त्री", "अध्यक्ष", "प्रदेश", "नेपाल", "नेपाली",
    "अभियान", "प्रदर्शन", "सडक", "क्षेत्र", "माग", "विरोध",
    "आन्दोलन", "समिति", "कार्यालय", "निर्देशन", "घोषणा",
}
...
def _filter_stories_by_districts(
    ...
    Checks structured district/province metadata first and only falls back to
    title matching for explicit district names when metadata is missing.
    """
    if not districts:
        return stories
...
def _build_feed_candidate(story) -> ClusterCandidate:
    features = _feed_feature_extractor.extract(
...
def _normalize_anchor_token(token: str) -> str:
    normalized = token.lower().translate(str.maketrans("०१२३४५६७८९", "0123456789"))
    normalized = normalized.replace("गङ्ग", "गंग")
    normalized = re.sub(r"(को|का|की|मा|बाट|ले|लाई|सँग|द्वारा|भित्र|अघि|पछि|हरू)$", "", normalized)
    normalized = re.sub(r"[^\w\u0900-\u097F]", "", normalized)
    return normalized.strip()


def _feed_anchor_tokens(candidate: ClusterCandidate) -> set[str]:
    anchors: set[str] = set()
    features = candidate.features
    if not features:
        return anchors
    for token in features.title_tokens:
        normalized = _normalize_anchor_token(token)
        if len(normalized) < 4:
            continue
        if normalized in _FEED_TITLE_IGNORE:
            continue
        anchors.add(normalized)
    return anchors


def _story_quality_rank(story) -> tuple:
    return (
        1 if getattr(story, "cluster_id", None) else 0,
        float(getattr(story, "relevance_score", 0) or 0),
        (story.published_at or story.created_at or datetime.min.replace(tzinfo=timezone.utc)).timestamp(),
    )


def _probable_feed_duplicate(
    story_a,
    candidate_a: ClusterCandidate,
    story_b,
    candidate_b: ClusterCandidate,
) -> bool:
    if story_a.id == story_b.id:
        return True
    if story_a.url and story_b.url and story_a.url == story_b.url:
        return True
    if getattr(story_a, "cluster_id", None) and getattr(story_b, "cluster_id", None) and story_a.cluster_id == story_b.cluster_id:
        return True

    published_a = story_a.published_at or story_a.created_at
    published_b = story_b.published_at or story_b.created_at
    if published_a and published_b:
        gap_hours = abs((published_a - published_b).total_seconds() / 3600.0)
        if gap_hours > 24:
            return False

    headline_score = ClusteringService._compute_same_event_headline_score(candidate_a, candidate_b)
    ...
    entities_a = set(features_a.title_entities or [])
    entities_b = set(features_b.title_entities or [])
    anchor_a = ClusteringService._headline_geo_anchor(candidate_a)
    anchor_b = ClusteringService._headline_geo_anchor(candidate_b)
    if entities_a and entities_b and (entities_a & entities_b) and anchor_a and anchor_b and anchor_a == anchor_b:
        return True

    shared_anchors = _feed_anchor_tokens(candidate_a) & _feed_anchor_tokens(candidate_b)
    if shared_anchors:
        longest_anchor = max(len(token) for token in shared_anchors)
        if longest_anchor >= 7:
            if anchor_a and anchor_b and anchor_a == anchor_b:
                return True
            if (published_a and published_b) and abs((published_a - published_b).total_seconds() / 3600.0) <= 8:
                return True

    return False


def _dedupe_consolidated_story_feed(stories: List) -> List:
    selected: list[tuple[object, ClusterCandidate]] = []
    for story in stories:
        candidate = _build_feed_candidate(story)
        duplicate_index: Optional[int] = None
        for idx, (kept_story, kept_candidate) in enumerate(selected):
            if _probable_feed_duplicate(story, candidate, kept_story, kept_candidate):
                duplicate_index = idx
                break

        if duplicate_index is None:
            selected.append((story, candidate))
            continue

        kept_story, kept_candidate = selected[duplicate_index]
        if _story_quality_rank(story) > _story_quality_rank(kept_story):
            selected[duplicate_index] = (story, candidate)
        else:
            selected[duplicate_index] = (kept_story, kept_candidate)

    return [story for story, _ in selected]


def _fallback_tracker_direction(development_stage: Optional[str]) -> str:
    """Map cluster lifecycle into the slower story-tracker direction labels."""
    return {
        "emerging": "rising",
        "developing": "rising",
        "mature": "stable",
        "resolved": "cooling",
    }.get((development_stage or "").lower(), "stable")


def _build_story_tracker_fallback_entries(cluster_rows: list[dict]) -> list[StoryTrackerEntry]:
    """Build story-tracker responses directly from cluster rows when narratives are missing."""
    fallback_entries: list[StoryTrackerEntry] = []

    for row in cluster_rows:
        cluster_id = row["cluster_id"]
        fallback_entries.append(
            StoryTrackerEntry(
                narrative_id=uuid5(NAMESPACE_URL, f"story-tracker-fallback:{cluster_id}"),
                label=row["headline"],
                thesis=row.get("bluf") or row["headline"],
                category=row.get("category"),
                direction=_fallback_tracker_direction(row.get("development_stage")),
                momentum_score=float(
                    row.get("urgency_score")
                    or ((row.get("source_count") or 0) * 5.0) + ((row.get("story_count") or 0) * 2.0)
                ),
                confidence=None,
                cluster_count=1,
                lead_regions=[
                    value
                    for value in [
                        row.get("primary_district"),
                        row.get("primary_province"),
                        row.get("primary_municipality"),
                    ]
                    if value
                ],
                lead_entities=list(row.get("main_entities") or []),
                first_seen_at=row.get("first_published"),
...
async def get_consolidated_stories(
    ...
    stories = _dedupe_consolidated_story_feed(stories)
...
async def get_developing_stories(
    ...
    Consumer reads never generate BLUFs on demand.
    """
    service = DevelopingStoriesService(db)
    rows = await service.list_entries(hours=hours, limit=limit, category=category, refresh=False)
...
async def get_story_tracker(
    ...
    Consumer reads never refresh narratives on demand.
    """
    service = StoryTrackerService(db)
    try:
        narratives = await service.list_narratives(hours=hours, limit=limit)
    except Exception:
        logger.warning("Story tracker narrative query failed; falling back to cluster-derived entries", exc_info=True)
        await db.rollback()
        narratives = []

    if not narratives:
        try:
            cluster_rows = await DevelopingStoriesService(db).list_entries(
                hours=hours,
                limit=limit,
                refresh=False,
            )
        except Exception:
            logger.warning("Story tracker fallback query failed", exc_info=True)
            await db.rollback()
            return []
        return _build_story_tracker_fallback_entries(cluster_rows)
