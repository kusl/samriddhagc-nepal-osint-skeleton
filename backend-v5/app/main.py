"""Nepal OSINT v5 - FastAPI Application."""
import asyncio
import logging
import time
...
from app.api.v1.websocket_map import router as ws_map_router
from app.routers.govt_scraper_router import router as govt_scraper_router
from app.core.realtime_bus import start_map_subscriber, start_news_subscriber
...
async def lifespan(app: FastAPI):
    ...
    from app.api.v1.websocket_map import manager as map_manager

    async def _broadcast_map(message: dict) -> None:
        await map_manager.broadcast(message, event_type=message.get("type"))

    tasks = [
        asyncio.create_task(start_news_subscriber(news_manager.broadcast)),
        asyncio.create_task(start_map_subscriber(_broadcast_map)),
    ]

    # Start background scheduler (RSS polling, scraping, clustering, etc.)
...
app = FastAPI(
    ...
    description="Clean, professional backend for Nepal OSINT platform",
    version="5.0.0",
    lifespan=lifespan,
)
...
class RequestTimingMiddleware(BaseHTTPMiddleware):
    ...
    Previous version opened a DB session per request — doubled connection pool
    pressure and caused QueuePool exhaustion under load (election day 2026-03-04).
    Now buffers in Redis and flushes every 60s in a background task.
    """

    _buffer: list[dict] = []
    _buffer_lock = None  # Initialized lazily (needs running event loop)
    _flush_task: asyncio.Task | None = None
    FLUSH_INTERVAL = 60  # seconds
    MAX_BUFFER = 500     # flush early if buffer gets large
...
# Include Map WebSocket router for real-time overlays
app.include_router(ws_map_router)

# Include Government Scraper router
app.include_router(govt_scraper_router)
