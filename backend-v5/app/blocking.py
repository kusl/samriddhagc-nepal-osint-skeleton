...
from app.services.clustering.feature_extractor import StoryFeatures, CITY_TO_DISTRICT
...
class HierarchicalBlocker:
    ...
    def __init__(self, time_window_hours: int = 24):
        ...
        self._title_anchor_ignore_tokens = {
    ...
    def get_candidate_pairs(
        ...
        by_category_day: Dict[str, List[UUID]] = defaultdict(list)
        by_title_entity: Dict[str, List[UUID]] = defaultdict(list)
        by_title_anchor: Dict[str, List[UUID]] = defaultdict(list)
        ...
        for story_id, features, category, published_at in stories:
            ...
            # Municipality names should also anchor into their parent district
            # so city-vs-district rewrites become candidate pairs.
            for municipality in features.municipalities:
                district = CITY_TO_DISTRICT.get(municipality)
                if district:
                    by_district[district].append(story_id)
            if features.primary_municipality:
                district = CITY_TO_DISTRICT.get(features.primary_municipality)
                if district:
                    by_district[district].append(story_id)

            for entity in features.title_entities:
                by_title_entity[entity].append(story_id)

            for token in self._title_anchor_tokens(features):
                by_title_anchor[token].append(story_id)
            ...
            if category and published_at:
                ...
                # Also keep a same-day bucket so sources that only expose a date
                # (midnight timestamps) can still match same-day rewrites.
                day_key = f"{category}_{published_at.date().isoformat()}_day"
                by_category_day[day_key].append(story_id)
        ...
        # Strong title entity pairs help catch same-person protest/arrest stories.
        for block in by_title_entity.values():
            if 2 <= len(block) <= 80:
                candidates.update(self._pairs_from_block(block))

        # Headline anchor pairs catch distinctive place/event rewrites even across categories.
        for block in by_title_anchor.values():
            if 2 <= len(block) <= 40:
                candidates.update(self._pairs_from_block(block))
        ...
        # Same-day category pairs catch date-only timestamps without forcing a full-day compare.
        for block in by_category_day.values():
            if len(block) >= 2:
                block = block[:60]
                candidates.update(self._pairs_from_block(block))
    ...
    def _pairs_from_block(self, block: List[UUID]) -> Set[Tuple[UUID, UUID]]:
        ...
        pairs = set()
        deduped_block = list(dict.fromkeys(block))
        for i, id1 in enumerate(deduped_block):
            for id2 in deduped_block[i + 1:]:
                if id1 == id2:
                    continue
    ...
    def _title_anchor_tokens(self, features: StoryFeatures) -> List[str]:
        anchors: List[str] = []
        for token in features.title_tokens:
            normalized = self._normalize_title_anchor_token(token)
            if len(normalized) < 4:
                continue
            if normalized in self._title_anchor_ignore_tokens:
                continue
            anchors.append(normalized)
        return sorted(set(anchors), key=lambda token: (-len(token), token))[:3]

    @staticmethod
    def _normalize_title_anchor_token(token: str) -> str:
...
class BlockingRules:
    ...
    def should_block_with_features(
        ...
        # Rule 2: Category mismatch - only hard block clearly incompatible pairs.
        # Same event reporting often drifts between political/social/economic labels
        # across Nepali outlets, so exact category equality is too strict here.
        if category1 and category2 and category1 != category2:
            incompatible_pairs = {
                ("disaster", "political"), ("political", "disaster"),
                ("disaster", "economic"), ("economic", "disaster"),
                ("sports", "political"), ("political", "sports"),
                ("sports", "crime"), ("crime", "sports"),
            }
        ...
        if has_nepal1 and has_nepal2:
            geo1 = self._expanded_nepal_geography(features1)
            geo2 = self._expanded_nepal_geography(features2)
    ...
    @staticmethod
    def _expanded_nepal_geography(features: StoryFeatures) -> Set[str]:
        geo = set(features.districts)
        if features.title_district:
            geo.add(features.title_district)
        for municipality in features.municipalities:
            geo.add(municipality)
            district = CITY_TO_DISTRICT.get(municipality)
            if district:
                geo.add(district)
        if features.primary_municipality:
            geo.add(features.primary_municipality)
            district = CITY_TO_DISTRICT.get(features.primary_municipality)
            if district:
                geo.add(district)
        return geo
