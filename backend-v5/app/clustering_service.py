...
class ClusteringService:
    ...
    SAME_EVENT_SUFFIXES = ("को", "का", "की", "मा", "बाट", "ले", "लाई", "सँग", "द्वारा", "भित्र", "अघि", "पछि", "हरू")
    SAME_EVENT_TITLE_REPLACEMENTS = (
        ("गङ्ग", "गंग"),
        ("ज्यान गुमाउनेको संख्या", "death"),
        ("मृत्यु हुनेको संख्या", "death"),
        ("मृतकको सङ्ख्या", "death"),
        ("मृतकको संख्या", "death"),
        ("संख्या चार पुग्यो", "4 death"),
        ("सङ्ख्या चार पुग्यो", "4 death"),
        ("संख्या 4 पुग्यो", "4 death"),
        ("सङ्ख्या 4 पुग्यो", "4 death"),
        ("ज्यान गुमाउने", "death"),
        ("निधन", "death"),
        ("मृत्यु", "death"),
        ("मृतक", "death"),
    ...
    async def _load_embeddings(self, candidates: List[ClusterCandidate]) -> None:
        ...
        Missing embeddings are generated in one bounded batch so fresh duplicates
        can cluster before the next scheduled embedding backfill runs.
        """
        ...
        if need_embedding:
            generated = await self._generate_missing_embeddings(need_embedding)
            if generated:
                logger.info(
                    "Generated %s missing embeddings during clustering run",
                    generated,
                )
            else:
                logger.info(
                    "Could not generate on-the-fly embeddings for %s stories; clustering will use lexical/structural fallback",
                    len(need_embedding),
                )

    async def _generate_missing_embeddings(self, candidates: List[ClusterCandidate]) -> int:
        if not candidates or not self.openai_runtime.embeddings_enabled:
            return 0

        bounded_candidates = sorted(
            candidates,
            key=lambda candidate: candidate.published_at or datetime.min.replace(tzinfo=timezone.utc),
            reverse=True,
        )[:80]

        texts: list[str] = []
        payloads: list[dict] = []
        active_candidates: list[ClusterCandidate] = []

        for candidate in bounded_candidates:
            text_parts = [candidate.title or "", candidate.summary or "", candidate.content or ""]
            text_value = " ".join(part.strip() for part in text_parts if part and part.strip())
            text_value = text_value[: self.settings.openai_max_embedding_chars_per_text].strip()
            if not text_value:
                continue
            ...
            payloads.append(
                {
                    ...
                    "model_version": f"openai-{self.settings.openai_embedding_dimensions}",
                }
            )
        ...
        try:
            embeddings = await self.openai_runtime.embed_texts(
                texts,
                model=self.settings.openai_embedding_model,
                dimensions=self.settings.openai_embedding_dimensions,
                user_scope="cluster-inline-embeddings",
            )
        except Exception as exc:
            logger.warning("Inline OpenAI embedding batch failed during clustering: %s", exc)
            return 0

        rows = []
        generated = 0
        for candidate, payload, embedding in zip(active_candidates, payloads, embeddings):
            if not embedding or all(x == 0.0 for x in embedding):
                continue
            candidate.embedding = list(embedding)
            rows.append(
                {
                    **payload,
    ...
    async def _hybrid_cluster(
        ...
        for id1, id2 in candidate_pairs:
            ...
            # If similar enough, union
            should_merge = (
                similarity.overall >= self.hybrid_threshold or
                self._should_force_merge_high_precision_pair(story1, story2, similarity)
            )
            if (
    ...
    def _should_force_merge_high_precision_pair(
        self,
        story1: ClusterCandidate,
        story2: ClusterCandidate,
        similarity: HybridSimilarityScore,
    ) -> bool:
        """Cheap deterministic merge for title-containment rewrites of the same event."""
        if similarity.blocked:
            return False

        same_event_headline_score = self._compute_same_event_headline_score(story1, story2)
        if same_event_headline_score >= 0.88:
            action1 = (story1.features.title_action if story1.features else None) or ""
            action2 = (story2.features.title_action if story2.features else None) or ""
            if action1 and action2 and action1 != action2 and same_event_headline_score < 0.95:
                return False
            time_gap_hours = self._pair_time_gap_hours(story1, story2)
            if time_gap_hours is not None and time_gap_hours > 12:
                return False
            return True

        if similarity.semantic < max(self.MIN_SEMANTIC_SIMILARITY, 0.64):
            return False
        if similarity.lexical < 0.90:
            return False
        ...
        title1 = self._normalize_title_for_containment(story1.title)
        title2 = self._normalize_title_for_containment(story2.title)
    ...
    @staticmethod
    def _normalize_title_for_containment(title: str) -> str:
        lowered = title.lower()
    ...
    def _should_run_openai_pair_check(
        ...
        if similarity.blocked:
            return False
        same_event_headline_score = self._compute_same_event_headline_score(story1, story2)
        headline_duplicate = same_event_headline_score >= 0.72
        entity_geo_candidate = False
        entities1 = set((story1.features.title_entities if story1.features else []) or [])
        entities2 = set((story2.features.title_entities if story2.features else []) or [])
        if entities1 and entities2 and (entities1 & entities2):
            anchor1 = self._headline_geo_anchor(story1)
            anchor2 = self._headline_geo_anchor(story2)
            if anchor1 and anchor2 and anchor1 == anchor2:
                entity_geo_candidate = True

        if similarity.semantic < self.MIN_SEMANTIC_SIMILARITY and not headline_duplicate and not entity_geo_candidate:
            return False
        ...
        near_threshold_lexical = (
            not cross_lingual and
            similarity.lexical >= 0.85 and
            similarity.structural >= 0.45 and
            similarity.overall >= max(0.58, self.hybrid_threshold - 0.12)
        )

        if similarity.overall > self.gray_zone_high:
            return False
        if (
            similarity.overall < self.gray_zone_low
            and not near_threshold_lexical
            and not headline_duplicate
            and not entity_geo_candidate
        ):
            return False

        return (
            cross_lingual
            or semantic_lexical_gap
            or low_structural
            or near_threshold_lexical
            or headline_duplicate
            or entity_geo_candidate
        )

    @classmethod
    def _normalize_title_for_same_event_signature(cls, title: str, district_hint: Optional[str] = None) -> tuple[set[str], set[str], str]:
        normalized = title.lower().translate(cls.NEPALI_DIGIT_TRANSLATION)
        for source, target in cls.SAME_EVENT_TITLE_REPLACEMENTS:
            normalized = normalized.replace(source, f" {target} ")
        for source, target in cls.NEPALI_NUMBER_WORDS.items():
            normalized = re.sub(rf"(?<!\S){re.escape(source)}(?=(?:को|जना|जनाको)?(?!\S))", target, normalized)
        normalized = re.sub(r"(\d+)(?:को|जना|जनाको)", r"\1", normalized)
        normalized = re.sub(r"[^\w\s\u0900-\u097F]", " ", normalized)
        normalized = re.sub(r"\s+", " ", normalized).strip()

        raw_tokens = normalized.split()
        numbers = {token for token in raw_tokens if token.isdigit()}
        tokens: set[str] = set()
        for token in raw_tokens:
            for suffix in cls.SAME_EVENT_SUFFIXES:
                if token.endswith(suffix) and len(token) > len(suffix) + 2:
                    token = token[: -len(suffix)]
                    break
            if district_hint and district_hint in token:
                continue
            if token in cls.SAME_EVENT_IGNORE_TOKENS:
                continue
            if len(token) <= 1:
                continue
            tokens.add(token)

        return tokens, numbers, normalized

    @classmethod
    def _compute_same_event_headline_score(cls, story1: ClusterCandidate, story2: ClusterCandidate) -> float:
        district1 = cls._headline_geo_anchor(story1)
        district2 = cls._headline_geo_anchor(story2)
        tokens1, numbers1, normalized1 = cls._normalize_title_for_same_event_signature(story1.title, district1)
        tokens2, numbers2, normalized2 = cls._normalize_title_for_same_event_signature(story2.title, district2)
        ...
        ratio_score = SequenceMatcher(None, normalized1, normalized2).ratio()
        number_score = 1.0 if numbers1 and numbers2 and (numbers1 & numbers2) else 0.0
        ...
        if district1 and district2 and district1 == district2:
            ...
            location_score = 0.0
        elif district1 or district2:
            location_score = 0.5

        event_score = 0.0
        event1 = (story1.features.event_type if story1.features else None) or (story1.features.title_action if story1.features else None)
        event2 = (story2.features.event_type if story2.features else None) or (story2.features.title_action if story2.features else None)
        ...
        same_event_context_bonus = 0.0
        if entity_score and location_score:
            action_pair = {event1 or "", event2 or ""}
            if "protest" in action_pair and ("arrest" in action_pair or "release" in action_pair):
                same_event_context_bonus = 1.0

        repeated_core = 1.0 if len(shared_tokens) >= 2 else 0.0
        ...
        return min(
            ...
            + 0.08 * entity_score
            + 0.05 * repeated_core
            + 0.12 * same_event_context_bonus,
        )

    @classmethod
    def _headline_geo_anchor(cls, story: ClusterCandidate) -> Optional[str]:
        if not story.features:
            return None
        if story.features.title_district:
            return CITY_TO_DISTRICT.get(story.features.title_district, story.features.title_district)
        if story.features.districts:
            return story.features.districts[0]
        if story.features.primary_municipality:
            return CITY_TO_DISTRICT.get(story.features.primary_municipality, story.features.primary_municipality)
        for municipality in story.features.municipalities:
            mapped = CITY_TO_DISTRICT.get(municipality)
            if mapped:
                return mapped
        return None

    @staticmethod
    def _pair_time_gap_hours(story1: ClusterCandidate, story2: ClusterCandidate) -> Optional[float]:
        if not story1.published_at or not story2.published_at:
            return None
        return abs((story1.published_at - story2.published_at).total_seconds() / 3600.0)
