...
class MarketService:
    ...
    async def get_latest_by_type(self, data_type: MarketDataType) -> Optional[MarketData]:
        ...
        result = await self.db.execute(
            ...
            .order_by(desc(MarketData.data_date), desc(MarketData.fetched_at))
            .limit(1)
        )
    ...
    async def ingest_all(self) -> dict:
        ...
        for key, task in (
            ...
            ("gold_silver", self.ingest_gold_silver),
            ("fuel", self.ingest_fuel),
            ("nepse", self.ingest_nepse),
        ):
            try:
                results[key] = await task()
            except Exception as exc:
                logger.error("Error ingesting %s: %s", key, exc)
