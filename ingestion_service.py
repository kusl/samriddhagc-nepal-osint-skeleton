...
class IngestionService:
    ...
    async def _generate_notifications_for_new_stories(self):
        """Generate consumer notifications for newly committed high-signal stories."""
        if not self._new_story_ids:
            return
        result = await self.db.execute(
            select(Story).where(Story.id.in_(self._new_story_ids))
        )
        stories = list(result.scalars().all())
        matcher = NotificationMatchingService(self.db)
        for story in stories:
            try:
                await matcher.process_story(story)
            except Exception as e:
                logger.warning(f"Failed to generate notifications for story {story.id}: {e}")
        self._new_story_ids = []

    async def after_commit(self):
        """Post-commit hooks for newly ingested stories."""
        await self._generate_notifications_for_new_stories()
    ...
    async def _process_article(self, article: FetchedArticle) -> str:
        ...
        try:
            ...
            self._new_story_ids.append(story.id)
