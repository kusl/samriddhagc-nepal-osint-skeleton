...
class ConsolidatedStoryResponse(BaseModel):
    ...
    def from_story(cls, story) -> "ConsolidatedStoryResponse":
        ...
        location_verified = bool(
            title_district
            or primary_municipality
            or (geo_confidence is not None and geo_confidence >= 0.5)
        )
        districts_affected = list(story.districts or []) if location_verified else []
        provinces_affected = list(story.provinces or []) if location_verified else []
        ...
        elif story_type == "political" and story.nepal_relevance == "NEPAL_DOMESTIC":
            # Ambiguous national political stories should not inherit regional
            # source branding; default them to Kathmandu instead.
